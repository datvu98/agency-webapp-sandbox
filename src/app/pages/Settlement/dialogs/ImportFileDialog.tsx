...
import { Button, Col, Flex, Modal, Row, Space, Spin, Upload } from "antd";
import { FileExcelTwoTone, InboxOutlined } from "@ant-design/icons";
import { useMutation } from "@apollo/client";
// import { FILE_IMPORT_PROD, FILE_IMPORT_STAG } from "../common/Constants";
import Axios from "axios";
import { showAlert, toAbsoluteUrl } from "utils/helper";
import mutate_cfImportDataSettlement from "graphql/mutations/mutate_cfImportDataSettlement";
import SVG from "react-inlinesvg";

const FILE_IMPORT_STAG = 'https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/Stagging/File_mau_doi_soat.xlsx'
const FILE_IMPORT_PROD = 'https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/prod/File_mau_doi_soat.xlsx'
...
const ImportFileDialog = ({ show, onHide, setDataImportMenual }) => {
  ...
  const [cfImportDataSettlement] = useMutation(mutate_cfImportDataSettlement, {
  ...
  const addProductFromFile = useCallback(async () => {
    ...
    if (!!data?.cfImportDataSettlement?.success) {
      ...
      showAlert.error(data?.cfImportDataSettlement?.message || 'Tải file thất bại');
    }
  }, [linkFile]);
      ...
      footer={[
        <Button
          ...
          style={{
            ...
            width: 100,
            background: '#ff5629',
            opacity: loadingSubmit || loading || !linkFile ? '0.6' : '',
            color: 'white'
          }}
          type="primary"
        >
    ...
    >
      <Space direction="vertical" size={15} className="w-100">
        ...
        <Row style={{ marginTop: 6 }}>
          <Col span={6}>
            Tải lên tập tin<span style={{ color: "red" }}>*</span>
          </Col>
        ...
        <Row style={{ marginTop: 6 }}>
          ...
          <Col span={18}>
            <div className="text-secondary-custom fs-14">
              File nhập có dung lượng tối đa 5MB và 1000 bản ghi
            </div>
          </Col>
        </Row>
      </Space>
      <Flex justify="center">
        <Spin spinning={loadingSubmit}></Spin>
      </Flex>
    </Modal>
  );
};
