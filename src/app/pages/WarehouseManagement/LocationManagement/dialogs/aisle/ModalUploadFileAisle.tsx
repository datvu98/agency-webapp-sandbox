...
const FILE_IMPORT_STAG = "https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/Stagging/Filemauthemluong.xlsx";
const FILE_IMPORT_PROD = "https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/prod/Filemauthemluong.xlsx";

const ModalUploadFileAisle = ({ currentWh, onHide, onShowModalFileUploadResults }) => {
	...
	const addLocationByFile = async () => {
		...
		if (data?.locationManagerCreateBulkByFile?.success == 1) {
			...
			showAlert.error(data?.locationManagerCreateBulkByFile?.message || "Nhập file thêm luống đi không thành công");
		}
	};
	...
	return (
		<Modal open={!!true} centered title={"Thêm luống đi qua file"} onCancel={resetData} footer={null} width={720}>
			<Spin spinning={loadingDataArea || loadingLocationManagerCreateBulkByFile}>
				...
				<div style={{ textAlign: "right", marginTop: 24 }}>
					<Button onClick={resetData} style={{ marginRight: 8 }} className="btn-base">
						Thoát
					</Button>
...
export default memo(ModalUploadFileAisle);
