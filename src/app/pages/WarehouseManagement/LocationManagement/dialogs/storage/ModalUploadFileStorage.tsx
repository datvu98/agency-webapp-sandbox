...
import mutate_storageEquipmentBulkCreateByFile from "graphql/mutations/mutate_storageEquipmentBulkCreateByFile";
const { Text } = Typography;
const FILE_IMPORT_STAG = "https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/Stagging/Filemauthemthietbi.xlsx";
const FILE_IMPORT_PROD = "https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/prod/Filemauthemthietbi.xlsx";

const ModalUploadFileStorage = ({ currentWh, onHide, onShowModalFileUploadResults }) => {
	const [linkFile, setLinkFile] = useState(null);
	const [fileName, setFileName] = useState(null);
	const [loading, setLoading] = useState(false);
	const inputRef = useRef<any>(null);

	const [storageEquipmentBulkCreateByFile, { loading: loadingStorageEquipmentBulkCreateByFile }] = useMutation(mutate_storageEquipmentBulkCreateByFile, {
	...
	const addLocationByFile = async () => {
		let { data } = await storageEquipmentBulkCreateByFile({
			variables: {
				created: {
		...
		if (data?.storageEquipmentBulkCreateByFile?.success == 1) {
			resetData();
			onShowModalFileUploadResults(data?.storageEquipmentBulkCreateByFile);
		} else {
			showAlert.error(data?.storageEquipmentBulkCreateByFile?.message || "Nhập file thêm thiết bị không thành công");
		}
	};
	...
	return (
		<Modal open={!!true} centered title={"Thêm thiết bị chứa qua file"} onCancel={resetData} footer={null} width={720}>
			<Spin spinning={loadingStorageEquipmentBulkCreateByFile}>
				<Row gutter={[16, 12]}>
					<Col span={6}>
				...
				<div style={{ textAlign: "right", marginTop: 24 }}>
					...
					<Button className="btn-base" type="primary" onClick={addLocationByFile} disabled={!fileName || loadingStorageEquipmentBulkCreateByFile}>
						Đồng ý
					</Button>
				</div>
			</Spin>
...
export default memo(ModalUploadFileStorage);
