...
const ModalAddSlot = ({ onHide, show, warehouseId }) => {
	const [form] = Form.useForm();

	const [createLocation, { loading: creating }] = useMutation(mutate_locationManagerCreate, {
	...
	const handleSubmit = async (values) => {
		try {
			...
			if (data?.locationManagerCreate?.success) {
				...
				showAlert.error(data?.locationManagerCreate?.message || "Tạo vị trí thất bại.");
			}
		} catch {
			showAlert.success("Đã xảy ra lỗi khi tạo vị trí.");
		}
	};

	return (
		<Modal open={show} onCancel={onHide} title="Thêm mới vị trí" centered width={900} footer={null} destroyOnClose>
			<Form form={form} layout="vertical" onFinish={handleSubmit} initialValues={{ quantity: 200 }}>
				<Row gutter={16}>
					<Col span={8}>
				...
				<Row gutter={16}>
					...
					<Col span={8}>
						...
						>
							<InputNumber className="w-100" placeholder="Nhập mức độ ưu tiên" size="large" />
						</Form.Item>
					</Col>
				</Row>
					...
					<Col span={8}>
						...
						>
							<InputNumber
								...
								className="w-100"
								placeholder="Nhập số lượng chứa tối đa"
								size="large"
							/>
						</Form.Item>
					</Col>
				...
				<Space style={{ width: "100%", justifyContent: "end" }}>
					...
					<Button type="primary" className="btn-base" htmlType="submit" loading={creating}>
						Tạo
					</Button>
				</Space>
			</Form>
