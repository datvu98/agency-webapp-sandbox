...
const FILE_IMPORT_STAG = "https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/Stagging/Filemauthemvitri.xlsx";
const FILE_IMPORT_PROD = "https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/prod/Filemauthemvitri.xlsx";

const ModalUploadFileSlot = ({ currentWh, onHide, onShowModalFileUploadResults }) => {
	...
	const addLocationByFile = async () => {
		...
		if (data?.locationManagerCreateBulkByFile?.success == 1) {
			...
			showAlert.error(data?.locationManagerCreateBulkByFile?.message || "Nhập file thêm vị trí không thành công");
		}
	};
	...
	return (
		<Modal open={!!true} centered title={"Thêm vị trí qua file"} onCancel={resetData} footer={null} width={720}>
...
export default memo(ModalUploadFileSlot);
