...
const RackManagementTable = ({ locationManagerUpdate, setShowDetail, data, loading, error, refetch }) => {
	...
	const columns = [
		...
		{
			title: "Mã luống đi",
			dataIndex: "aisle_code",
			key: "aisle_code",
			width: 150,
			render: (_item, record) => {
				return <Text>{record?.aisle?.code}</Text>;
			},
		},
		{
			title: "Trạng thái",
						...
						switch (key) {
							...
							case "inactive":
								...
								if (dataInactive?.locationManagerUpdate?.success) {
									showAlert.success("Dừng hoạt động kệ thành công.");
								} else {
									showAlert.error(dataInactive?.locationManagerUpdate?.message || "Dừng hoạt động kệ thất bại.");
								}
								break;
							...
							case "active":
								...
								if (dataActive?.locationManagerUpdate?.success) {
									showAlert.success("Bật hoạt động kệ thành công.");
								} else {
									showAlert.error(dataActive?.locationManagerUpdate?.message || "Bật hoạt động kệ thất bại.");
								}
								break;
							default:
								break;
...
export default RackManagementTable;
