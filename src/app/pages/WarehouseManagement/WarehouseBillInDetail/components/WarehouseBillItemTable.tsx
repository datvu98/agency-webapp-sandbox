...
const WarehouseBillInItemTable = ({ dataDetail }) => {
    ...
    const columns = [
		...
		},
		{
			...
			align: "center",
			render: (_item, record) => {
				return <Text>{record?.expiredInfo?.lotSerial || '--'}</Text>;
			},
		},
		{
			title: "Hạn sử dụng",
			...
			render: (_item, record) => {
				return (
					<Text>{record?.expiredInfo?.expiredDate ? dayjs.unix(record?.expiredInfo?.expiredDate).format('DD/MM/YYYY HH:mm') : '--'}</Text>
				);
			},
		},
		{
		...
		{
			title: "Số lượng đã lưu kho",
			dataIndex: "quantityPutAway",
			key: "quantityPutAway",
			width: 200,
			align: "center",
			render: (_item, record) => {
...
export default WarehouseBillInItemTable;
