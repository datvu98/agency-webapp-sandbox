...
import { FileExcelTwoTone, FileOutlined, InboxOutlined } from "@ant-design/icons";
...
import mutate_warehouseBillUpdateById from "graphql/mutations/mutate_warehouseBillUpdateById";

const ModalUpdateEvidence = ({ show, onHide, dataDetail }) => {
	...
	const [warehouseBillUpdateById] = useMutation(mutate_warehouseBillUpdateById, {
		awaitRefetchQueries: true,
		refetchQueries: ["warehouseBillGetById"],
	});

	const updateEvidence = useCallback(async () => {
		setLoadingSubmit(true);
		let { data } = await warehouseBillUpdateById({
			variables: {
				id: dataDetail?.id,
                updated: {
                    evidenceUrl: linkFile
                }
			},
		});
		setLoadingSubmit(false);
		if (!!data?.warehouseBillUpdateById?.success) {
            showAlert.success("Cập nhật chứng từ thành công");
			resetData();
		} else {
			showAlert.error(data?.warehouseBillUpdateById?.message || "Cập nhật chứng từ thất bại");
		}
	}, [linkFile]);
	...
	return (
		...
		>
			<Space direction="vertical" size={15} className="w-100">
				<Row>
					<Col span={6}>Mã phiếu nhập kho</Col>
					<Col span={18}>{dataDetail?.code}</Col>
				</Row>
				<Row style={{ marginTop: 6 }}>
					<Col span={6}>
						Tải lên chứng từ<span style={{ color: "red" }}>*</span>
					</Col>
					<Col span={18} style={{ height: 150, alignItems: "center", justifyContent: "center", display: "flex", border: "1px solid rgba(0, 0, 0, 0.2)", borderRadius: 5 }}>
						<input accept=".png, .jpeg, .xls, .pdf, .docx, .jpg" style={{ display: "none" }} type="file" onChange={handleFileChange} ref={inputRef} />
						<div style={{ textAlign: "center" }}>
							...
							{!linkFile && !loading && (
								<>
									...
									<div style={{ fontSize: 14, marginBottom: 8 }}>File dưới 5MB</div>
								</>
							)}
							{linkFile && (
								<>
									<FileOutlined style={{ fontSize: 70 }} />
				...
				<Row style={{ marginTop: 6 }}>
					<Col span={6}></Col>
					<Col span={18}>
						<div className="text-secondary-custom fs-14">File nhập có dung lượng tối đa 5MB</div>
					</Col>
				</Row>
			</Space>
			<Flex justify="center">
...
export default memo(ModalUpdateEvidence);
