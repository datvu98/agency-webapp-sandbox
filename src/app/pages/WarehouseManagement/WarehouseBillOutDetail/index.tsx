...
import { WarehouseBillOutDetailWrapper } from "../Warehouse.styles";
...
import WarehouseBillOutItemTable from "./components/WarehouseBillItemTable";
import ProcessingInfo from "./components/ProcessingInfo";

const { Text } = Typography;

const WarehouseBillOutDetail = () => {
	...
	// const optionPIC = useMemo(() => {
	// 		return [{
	// 			label: `(Admin) ${user?.email}`,
	// 			value: `admin_${user.id}`,
	// 			is_subuser: 0
	// 		}, ...optionSubUsers,]
	// 	}, [optionSubUsers, user])

	return (
		<WarehouseBillOutDetailWrapper>
			<Helmet titleTemplate="Chi tiết phiếu xuất kho" defaultTitle="Chi tiết phiếu xuất kho">
				<meta name="description" content="Chi tiết phiếu xuất kho" />
			</Helmet>
			<Spin spinning={loadingWarehouseBillGetById}>
					...
					<Col span={24}>
						<Card>
							<Row gutter={10}>
								<Col span={18}>
									<Flex gap={10} vertical>
										...
										/>
										<WarehouseBillOutItemTable dataDetail={warehouseBillGetById?.warehouseBillGetById?.data} />
									</Flex>
								</Col>
                                <Col span={6}>
                                <ProcessingInfo dataDetail={warehouseBillGetById?.warehouseBillGetById?.data} 
							...
							<Flex justify="end" gap={10} style={{ marginTop: 10 }}>
								<Button
									onClick={() => {
										navigate(`/${user?.category_code == 'fulfillment' ? 'outbound-manage' : "warehouse-manage"}/warehouse-bill-out`);
									}}
...
export default WarehouseBillOutDetail;
