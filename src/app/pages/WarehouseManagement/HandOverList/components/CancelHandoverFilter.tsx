...
import { OPTION_CANCEL_HANDOVER_TIME, OPTION_SHIPPING_CARRIERS, OPTION_STATUS, OPTION_TIME } from "../constant";

const { Text } = Typography;
const { RangePicker } = DatePicker;

const CancelHandoverFilter = ({optionsWarehouse}) => {
    ...
    return (
        <>
                    ...
                    <Row align={"middle"}>
                        <Col span={8}>
                            <Select
                                ...
                                options={OPTION_CANCEL_HANDOVER_TIME}
                                value={params?.date_search_type || OPTION_CANCEL_HANDOVER_TIME[0]?.value}
                            />
                        </Col>
                        <Col span={16}>
                            <RangePicker
            ...
            <Row style={{ marginTop: 10, marginBottom: 20 }} gutter={20} align="middle">
                <Col span={16}>
                    <Input
                        placeholder="Tìm kiếm mã kiện hàng, mã vận dơn..."
                        value={searchText}
                ...
                <Col span={8}>
                    <Select
                        className="w-100"
                        placeholder="Kho"
                        value={params?.warehouse ? Number(params?.warehouse) : optionsWarehouse?.[0]?.value}
                        ...
                        onChange={(value: any) => {
                            if (!!value) {
                                ...
                                navigate(`${location.pathname}?${queryString.stringify(omit(params, ["warehouse"]))}`);
                            }
                        }}
                    />
                </Col>
...
export default memo(CancelHandoverFilter);
