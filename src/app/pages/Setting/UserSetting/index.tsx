...
const UserSetting = () => {
    ...
    const UserInfoRow = ({ label, value }) => (
        <Row style={{ marginBottom: '10px' }}>
            <Col span={12}>
                <Flex vertical style={{ textAlign: 'right', height: '100%' }} gap={8} justify="center">
                    <Text strong>{label}</Text>
                </Flex>
            </Col>
            <Col span={1}></Col>
            <Col span={11}>
                <Flex vertical style={{ textAlign: 'left', height: '100%' }} gap={8} justify="center">
                    <Tooltip title={value}>
            ...
            <Card className="card-switch" title={false}>
                ...
                <Row style={{ marginTop: '20px' }}>
                    <Col span={10} style={{ margin: 'auto' }}>
                        ...
                        <UserInfoRow label="Hình thức tổ chức" value="Doanh nghiệp" />
                        <UserInfoRow label="Tên doanh nghiệp" value={user?.full_name} />
                        <UserInfoRow label="Mã tài khoản" value={user?.agencyId} />
                    </Col>
                    <Col span={8}>
                    ...
                    <Col span={6}>
                        <Flex gap={12} justify="flex-end" style={{ marginRight: '50px' }}>
                            <Button
                                type="primary"
                                className="btn-base"
                                onClick={() => { navigate('/settings/user/change-password') }}
                            >
                                Đổi mật khẩu
                            </Button>
                            <Button
                                type="primary"
                                onClick={() => { navigate('/settings/user/update') }}
                                className="btn-base"
                            // loading={loading}
