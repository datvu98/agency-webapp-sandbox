...
import { PaginationModalWrapper } from "./PaginationModal.style";
...
export default memo(
	({
		...
		const optionsPagination = useMemo(() => {
			if (optionPagination.length > 0) return optionPagination;
			if (lite) return [{ label: 10, value: 10 }];
			if (!quickAdd)
				return [
					{ label: 25, value: 25 },
					{ label: 50, value: 50 },
			...
			if (isAddReserve) return [{ label: 20, value: 20 }];
			return [
				{ label: 24, value: 24 },
				{ label: 48, value: 48 },
				{ label: 72, value: 72 },
		...
		}, [quickAdd, isAddReserve, lite, optionPagination]);
		if (totalPage <= 0) {
			return (
				<>
					{!loading && isShowEmpty && (
						<div className="text-center my-8">
							<span className="text-muted">{emptyTitle}</span>
						</div>
					)}
				...
				<div className="d-flex justify-content-between align-items-center flex-wrap row">
					<div className="d-flex align-items-center py-3 px-3 col-6">
						...
						{onSizePage && (
							...
							>
								{optionsPagination?.map((_option) => (
									<option key={`option-pagination-${_option.value}`} value={_option.value}>
						...
						<span className="text-muted col-7">
							Hiển thị {(page - 1) * limit + 1} - {(page - 1) * limit + count} {!!totalRecord ? `của ${totalRecord}` : ""}
						</span>
					</div>
					<div className="d-flex flex-wrp py-2 col-6 justify-content-end">
						...
						{[page - 1, page, page + 1, page + 2]
							.filter((_page) => _page > 0 && _page <= totalPage)
							.map((_page, idx) => {
								if (_page == page) {
									return (
										<a
